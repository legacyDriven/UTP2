...
public class Main {
  ...
  public static void test() {
    ...
    System.out.println(m1);   //  Maybe has value aaa
    s = null;
    Maybe<String> m2 = Maybe.of(s);
    System.out.println(m2);   //  Maybe is empty
    ...
    m5.ifPresent(System.out::println);   //10

    // Metoda map()
    Maybe<Integer> m6 = m5.map( n -> n + 10 );
    System.out.println(m6);  //   Maybe has value 20

    // Metoda get()
    System.out.println(m6.get());  //20
    try {
      System.out.println(m4.get());  //  java.util.NoSuchElementException:  maybe is empty
    } catch(Exception exc) {
